...
<script type="text/javascript">
	...
	function search() {
		...
		$
				.getJSON(
						'${pageContext.request.contextPath}/wrapperAction_getWrappers',
						postParam,
						function(data, status) {
							if (data.retcode == "0000") {
										...
										.each(
												...
												function(index, item) {
													...
													addHtml += "<tr>" + "<td>"
														...
														if(item.status=="2"){
															addHtml+="<a onclick='checkPackagingMaterial("+item.id+")' class='btn-operation' href='#' title='审核'><span class='glyphicon glyphicon-thumbs-up' aria-hidden=‘true’></span></a>";
														}
														addHtml	+= "</td></tr>";
												});
	...
	function del(tid) {
		...
		$
				.getJSON(
						'${pageContext.request.contextPath}/wrapperAction_deleteWrapperDetail ',
						postParam, function(data, status) {
							if (data.retcode == "0000") {
								search();
							} else {
