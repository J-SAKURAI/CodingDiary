...
<div class="wrapper wrapper-content animated fadeInRight">
    ...
    <table class="table">
        ...
        <tr>
            <td colspan="6">
                <strong id="title">农家使用情况</strong>
            </td>
            </tr>
            <tr>
                        <td>
                <a class="btn btn-primary" href="${pageContext.request.contextPath}/web/jsp/management/peasant.jsp">返回</a>
                <a class="btn btn-primary" onclick="printExcelForPeasantFamilyUsage()">导出Excel</a>
                <a type="hidden" href="" id="printExcelForPeasantFamilyUsage" download="农家使用情况.xls"><span id="printExcelForPeasantFamilyUsageSpan"></span></a>
            </td>
        </tr>
        </tbody>
    </table>
</form>
<table class="table" id="peasantUsage">
    <thead>
    <tr>
    	<td>序号</td>
        <td>活动标题</td>
    ...
    <tr id="selectTemplete" style="display:none">
    	<input id="peasant_id" name="peasant_id" type="hidden" value="" />
...
$(function() {		
	...
	$.ajax({
        ...
        url: "${pageContext.request.contextPath}/peasantOperationAction_getPeasantFamilyDetail",
        cache: false,
        success: function(data) {
        	if(data.retcode=="0000"){
        			$("#title").text(data.obj.name+"的使用情况");
        	}
        	else{
        		alert(data.errorMsg);
        	}
    ...
    var aoColumns = [ 
		...
		var ajaxSource = {
			"select" : "${pageContext.request.contextPath}/peasantOperationAction_getPeasantFamilyUsage",
		};
		initTable("peasantUsage", aoColumns, ajaxSource);
});
...
function printExcelForPeasantFamilyUsage(){
	$.ajax({
        type: "post",
        async: false,
        dataType: "json",
        ...
        url: "${pageContext.request.contextPath}/peasantOperationAction_printExcelForPeasantFamilyUsage",
        cache: false,
        success: function(data) {
        	if(data.retcode=="0000"){
        		$("#printExcelForPeasantFamilyUsage").attr("href","../../download/"+data.obj);	
        		$("#printExcelForPeasantFamilyUsageSpan").trigger("click");
        	}
        	else{
        		alert(data.errorMsg);
        	}
