                            ...
                            <div class="ibox-content">
                                <div class="form-horizontal m-t" id="formid">
                                	...
                                	<div class="form-group">
                                        <label class="col-sm-2 control-label">原主图：</label>
                                        <div class="col-sm-10">
                                        	<img id="old_pic" src="" style="width:400px;height:200px">
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-10">
                                            ...
                                            <span class="help-block m-b-none"><i class="fa fa-info-circle"></i>用于显示的图片1，最大10MB，支持jpg/gif(如不需要修改，置为空即可)</span>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        <label class="col-sm-2 control-label">集合地点：</label>
                                        <div class="col-sm-10">
                                    ...
                                    <div class="form-group">
                                        <div class="col-sm-8 col-sm-offset-3">
                                            <button id="submitid" onclick="editActivity()">提交</button><!--type="submit"--><!--class="btn btn-primary"  -->
                                        </div>
                                    </div>
...
$(document).ready(function() {
	...
	$.ajax({
        ...
        //收受数据格式
        data: data,
        url: "${pageContext.request.contextPath}/activityOperationAction_getActivity",
        cache: false,
        success: function(data) {
            if(data.retcode=="0000"){
            	var startTime=data.obj.activity.startTime.substring(0, 10).replace(/-/g,"/");
            	var endTime=data.obj.activity.endTime.substring(0, 10).replace(/-/g,"/");
            	var signUpEndTime=data.obj.activity.signUpEndTime.substring(0, 16).replace(/-/g,"/");
            	signUpEndTime=signUpEndTime.replace("T"," ");
            	var meetingTime=data.obj.activity.meetingTime.substring(0, 16).replace(/-/g,"/");
            	meetingTime=signUpEndTime.replace("T"," ");
            	$("#old_pic").attr("src","${pageContext.request.contextPath}"+data.obj.activity.mainPicRoute);
    			$("#title").val(data.obj.activity.title);
    			$("#start_time").val(startTime);
    			$("#end_time").val(endTime);
    			$("#sign_up_end_time").val(signUpEndTime);
    			$("#meeting_time").val(meetingTime);
    			var meeting_places="";
    			$.each(data.obj.meetingPlaces, function(index, item) {
    				meeting_places+=item.placeName+"/";
    			});
    			meeting_places=meeting_places.substring(0,meeting_places.length-1);
    			$("#meeting_place").val(meeting_places);
    			$("#original_price").val(data.obj.activity.originalPrice);
    			$("#off_price").val(data.obj.activity.offPrice);
    			$("#destination").val(data.obj.activity.destination);	
    			$("#cost_component").val(data.obj.activity.costComponent);
    			$("#max_people").val(data.obj.activity.maxPeople);
    			$("#out_people").val(data.obj.activity.outPeople);
    			$("#attendance").val(data.obj.activity.attendance);
    			$("#type").val(data.obj.activity.type);	
    			$("#description1").val(data.obj.activity.description1);
        		$("#description2").val(data.obj.activity.description2);
        		$("#description3").val(data.obj.activity.description3);
        		$("#tag1").val(data.obj.activity.tag1);
        		$("#tag2").val(data.obj.activity.tag2);
        		$("#tag3").val(data.obj.activity.tag3);
        		$("#display-school").val(data.obj.activity.whetherDisplaySchool);
        		getTypes(data.obj.type.length);
    			$.each($("select[name='type']"), function(index, item) {
					$(item).val(data.obj.type[index].id);
				});
    			getEquipments(data.obj.equipment.length);
    			$.each($("select[name='equipment']"), function(index, item) {
					$(item).val(data.obj.equipment[index].id);
				});
				getRewards(data.obj.rewards.length);
    			$.each($("select[name='reward']"), function(index, item) {
					$(item).val(data.obj.rewards[index].id);
				});
				getLeaders(data.obj.leader.length);
    			$.each($("select[name='leader']"), function(index, item) {
					$(item).val(data.obj.leader[index].id);
				});
				getPeasantFamilies(data.obj.peasant.length);
    			$.each($("select[name='peasant']"), function(index, item) {
					$(item).val(data.obj.peasant[index].id);
				});
				getCars(data.obj.car.length);
    			$.each($("select[name='car']"), function(index, item) {
					$(item).val(data.obj.car[index].id);
				});
				editor1.$txt.html(data.obj.activity.preHtml1);
				editor2.$txt.html(data.obj.activity.preHtml2);
				editor3.$txt.html(data.obj.activity.preHtml3);
				editor4.$txt.html(data.obj.activity.preHtml4);
        	}
        	else{
    ...
    function getTypes(count){
    	for(var i=0;i<count;i++){
        	...
        	html+="<option value='9'>特价</option></select><a class='fa fa-remove' onclick='removeItem(this)'></a>";        	
        	$("#type").append(html);        	
        }
        $("#type").append("<a class='fa fa-plus' onclick='add(this)'></a>");
    }
      
    function getEquipments(count){
    	$.ajax({
        type: "post",
        async: false,
        dataType: "json",
        ...
        success: function(data) {
            if(data.retcode=="0000"){
        		...
        		html+="</select><a class='fa fa-remove' onclick='removeItem(this)'></a>";
        		$("#equipment").append(html);
        		}
        		$("#equipment").append("<a class='fa fa-plus' onclick='add(this)'></a>");
        	}
        	else{
        		showMessage(data.errorMsg);
        	}
    ...
    function getRewards(count){
        ...
        success: function(data) {
            if(data.retcode=="0000"){
        		...
        		$("#reward").append("<a class='fa fa-plus' onclick='add(this)'></a>");
        	}
        	else{
        		showMessage(data.errorMsg);
        	}
    ...
    function getCars(count){
        ...
        success: function(data) {
            if(data.retcode=="0000"){
        		...
        		$("#car").append("<a class='fa fa-plus' onclick='add(this)'></a>");
        	}
        	else{
        		showMessage(data.errorMsg);
        	}
    ...
    function getPeasantFamilies(count){
    	$.ajax({
        type: "post",
        async: false,
        dataType: "json",
        ...
        success: function(data) {
            if(data.retcode=="0000"){
        		...
        		$("#peasant").append("<a class='fa fa-plus' onclick='add(this)'></a>");
        	}
        	else{
        		showMessage(data.errorMsg);
        	}
    ...
    function getLeaders(count){
        ...
        success: function(data) {
            if(data.retcode=="0000"){
        		...
        		$("#leader").append("<a class='fa fa-plus' onclick='add(this)'></a>");
        	}
        	else{
        		showMessage(data.errorMsg);
        	}
    ...
    function editActivity(){
        ...
        html1=html1.replace(/<p><img/g, "<img");
        html1=html1.replace(/<p><br><\/p>/g, "<br>");
        html1=html1.replace(/<p>/g, "<br>");
        html1=html1.replace(/<\/p>/g, "");
        html1=html1.replace(/<\/a><br>/g, "</a>");*/
		var data = new FormData();
		var has_pic=0;
		if($("#main_pic")[0].files[0]!=null){
        ...
        //收受数据格式
        data: data,
        url: "${pageContext.request.contextPath}/activityOperationAction_editActivity",
        cache: false,
        processData: false,
    	contentType: false,
        success: function(data) {
