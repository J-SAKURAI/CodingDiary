	...
	function initOrderPropoty(tableId){
		...
		if($(selectTemplete).find("#orderPropoty").length==0){
			$(selectTemplete).append("<input id='orderPropoty' type='hidden'/>");
		}
		if($(selectTemplete).find("#orderType").length==0){
			...
			success: function(data){
				if(data.retcode=="0000"){
					...
					$.each(list, function(index, item) {
						...
						//自己增加
						$(createObj).find("#speciala").attr("href","../management/carUsage.jsp?id="+item.id);
						$(createObj).find("#peasantspeciala").attr("href","../management/peasantUsage.jsp?id="+item.id);
						$.each(columns[tableId], function(index1, item1) {
							...
							if(type=="INPUT"){
								...
								var context=url.split("/web")[0];
								console.log(context);
								$(createObj).find("#"+mDataProp).attr("src",context+item[mDataProp]);
							}
							else{
								if(mDataProp=="startTime"){
									var temp=item[mDataProp].substring(0,10);
									item[mDataProp]=temp;
								}
								if(mDataProp=="endTime"){
									var temp=item[mDataProp].substring(0,10);
									item[mDataProp]=temp;
								}
...
//						console.log(orderType);
	...
	function submitAddForFile(obj,div){
		...
		if(div==null){
			...
			$.each(columns[tableId], function(index, item) {
				...
				if($(obj).parents("tr").find("#"+mDataProp).attr("type")=="file"){
					if($(obj).parents("tr").find("#"+mDataProp)[0].files[0]==null){
						alert("请选择图片！");
						flag=1;
						return;
					}
					postParams.append("image", $(obj).parents("tr").find("#"+mDataProp)[0].files[0]);
				}
				else{
					postParams.append(mDataProp, value);
				}
			});
			if(flag==1){
				return;
			...
			$.each(inputs, function(index, item) {
				...
				if(sourceName!=null){
					...
					postParams.append(param, JSON.stringify(params));
				}
				if(sourceName==null&&sourceId==null){
					postParams.append(param, $(item).val());
				}
			});
		}
		$.ajax({
	...
	function myDeleteItem(obj){
		...
		var tableId=$(obj).parents("table").attr("id");
		var itemId=value=$(obj).parents("tr").find("#itemId").val();
		var order_id=$(obj).parents("tr").find("#order_id").val();
		var postParam = {
			"itemId" : itemId,
			"order_id" : order_id
		};
					...
					if(div==null){
						...
						$.each(columns[tableId], function(index, item) {
							...
							if(type=="INPUT"){
								...
								//$(createObj).find("#"+mDataProp+" option[value='"+item[mDataProp]+"']").attr("selected","selected");
								var params=jQuery.parseJSON($(createObj).find("#"+mDataProp).attr("params"));
								$(createObj).find("#"+mDataProp).html(params[value]);
							}else{
						...
						$.each(columns[tableId], function(index, item) {
							...
							if(component.length>0){
								if(type=="INPUT"){
									...
									//$(createObj).find("#"+mDataProp+" option[value='"+item[mDataProp]+"']").attr("selected","selected");
									var params=jQuery.parseJSON($(div).find("#"+div).attr("params"));
									$(div).find("#"+mDataProp).html(params[item[mDataProp]]);
								}else{
	...
	function submitEditForFile(obj){
		var postParams = new FormData();
		var tableId=$(obj).parents("table").attr("id");
		var itemId=value=$(obj).parents("tr").find("#itemId").val();
		postParams.append("itemId",itemId);
		$.each(columns[tableId], function(index, item) {
			var mDataProp=item.mDataProp;
			var type=$(obj).parents("tr").find("#"+mDataProp).prop("nodeName");
			var value="";
			...
			if($(obj).parents("tr").find("#"+mDataProp).attr("type")=="file"){
				...
				if($(obj).parents("tr").find("#"+mDataProp)[0].files[0]!=null){
					postParams.append("image", $(obj).parents("tr").find("#"+mDataProp)[0].files[0]);
					has_pic=1;
					postParams.append("has_pic",has_pic);
				}
				else{
					has_pic=0;
					postParams.append("has_pic",has_pic);
	...
	function initPageDiv(tableId){
		...
		if(pageDiv!=null&&pageDiv.length>0){
			pageDiv.html("<div class='table-page-left'>"+
					...
					//"<li id='nexts'><a onclick=\"nextsPage('"+tableId+"')\">>></a></li>" +
					"</ul></nav></div>");
